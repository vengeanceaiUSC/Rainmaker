...
def bake_equations_into_dcf(wb, *, tax_rate: float | None = None) -> None:
    ...
    ws = wb[SHEET_DCF]
    # MODEL20: Yacktman credit-adjusted β (not raw Yahoo 1.32)
    from .wacc import (
        BETA_AAA_COUPON,
        BETA_BLUME,
        BETA_RAW,
    ...
    # ----- Header -----
    ws["Q2"] = "vengeanceaiUSCMODEL20.0 — LIVE EQUATIONS + SOURCE LINKS"
    ws["Q2"].font = HDR
    ws["Q2"].fill = HDR_FILL
    ...
    ws["C6"] = (
        f"MODEL20: Yacktman-adj CAPM β={w.beta:.3f} (Yahoo raw {_B_RAW:.2f}); "
