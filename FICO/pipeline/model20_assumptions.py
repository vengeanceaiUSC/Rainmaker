"""vengeanceaiUSCMODEL20.0 — Fix levered-buyback debt in equity bridge on MODEL18.

MODEL18 rated 7.5/10: stub / SBC / SaaS fixes work, but levered 1.4×FCF buybacks
shrink DCF shares (~21.6M→~11.0M) while Equity = EV + Cash − Debt still subtracts
ONLY today's 10-Q debt (D34=D13). The extra 0.4×FCF is debt-funded on 3S rows
19–20, so future debt never hits the equity bridge and $/share is overstated.

MODEL20.0 package (keeps MODEL18 operating/stub/WACC assumptions):
  1. Same stub clock, SBC, SaaS, CapEx, Yacktman-adj CAPM as MODEL18
  2. Equity bridge debt D34 = D13 + stub×Y1 debt CF + Y2–Y5 debt CF (3S 19)
  3. $/share still ÷ Y5 buyback-adjusted shares (I16) — now paired with that debt
  4. Cash bridge stays current 10-Q (financing keeps ΔCash≈0 by design)
"""
...
MODEL_NAME = "vengeanceaiUSCMODEL20.0"

# --- MODEL18 critique (Phase 1) ---
MODEL18_RATING = "7.5/10"
MODEL18_CRITIQUE = (
    "MODEL18 flaws affecting Equity→$/share: (a) 1.4×FCF buybacks cut DCF shares "
    "to ~11M while D34 still equals today's 10-Q debt only; (b) 3S debt issuance "
    "(row 19 = (MULT−1)×FCF) funds the extra 0.4× and is never subtracted from "
    "Equity Value — overstates intrinsic $/share. Stub/SBC/SaaS/WACC keep."
)
MODEL17_RATING = "7.0/10"
MODEL17_CRITIQUE = MODEL18_CRITIQUE  # retained alias for shared explanation helpers
...
# --- Yacktman-adjusted CAPM (primary D6) — same algebra as MODEL17/18 ---
MODEL18_WACC = MODEL17_CAPM_WACC
MODEL20_WACC = MODEL18_WACC
...
ASSUMPTIONS_PDF_FILENAME = "MODEL20_FICO_ASSUMPTIONS_LIST.pdf"
ASSUMPTIONS_PDF_URL = (
    "https://raw.githubusercontent.com/vengeanceaiUSC/Rainmaker/"
    f"cursor/vengeanceaiusmodel20-d3ac/FICO/output/{ASSUMPTIONS_PDF_FILENAME}"
)
...
SOURCE_LINKS: Dict[str, str] = {
    **_M5_LINKS,
    ...
    "MODEL20 Assumptions List (PDF download)": ASSUMPTIONS_PDF_URL,
    "MODEL20 Assumptions List (PDF view)": ASSUMPTIONS_PDF_VIEW_URL,
}
...
FINANCING_METHODOLOGY_NOTE = (
    f"Buybacks unchanged vs Model18 ({BUYBACK_FCF_MULTIPLE:.2f}×(CFO−CapEx); "
...
DEBT_BRIDGE_METHODOLOGY_NOTE = (
    "Updated from Model18 (Previous: D34=D13 today's 10-Q debt only while I16 "
    "used Y5 buyback-shrunk shares) to Model20 (New: D34 = D13 + stub×3S!J19 + "
    "K19+L19+M19+N19). Y1 debt CF stub-scaled to match stub-scaled Y1 buybacks. "
    "Cash D33 stays 10-Q cash+mkt (ΔCash≈0 by design). Pairs levered share "
    "shrink with the debt that funded it."
)

DILUTION_METHODOLOGY_NOTE = (
    f"Shares path unchanged vs Model18 ({SHARES_OUTSTANDING_000s:,.3f}k @ "
    f"${SHARE_PRICE:,.0f} post–Q3 → Y5 via 1.4×FCF). SBC add-back does NOT "
    f"dilute; Y1 share reduction uses stub-scaled equity CF "
    f"({STUB_FRACTION_REMAINING:.2%} of year). MODEL20 pairs I16 with D34 debt "
    f"that includes forecast issuance."
)
...
YACKTMAN_METHODOLOGY_NOTE = (
    f"Yacktman lens (MODEL18 rated {MODEL18_RATING}): {MODEL18_CRITIQUE} "
    f"MODEL20 keeps AAA-coupon β ({BETA_YACKTMAN:.3f}), stub, SBC, SaaS mix, "
    "and subtracts levered-buyback debt in the equity bridge. "
...
def cover_blurb() -> str:
    return (
        f"{MODEL_NAME}: MODEL18={MODEL18_RATING}; "
        f"D34+=forecast debt from 1.4×FCF buybacks; "
        f"stub {STUB_FRACTION_REMAINING:.1%} of Y1 FCFF from {VALUATION_DATE.isoformat()}; "
        f"WACC Yacktman-adj {MODEL20_WACC:.2%} (β {BETA_YACKTMAN:.3f}); "
