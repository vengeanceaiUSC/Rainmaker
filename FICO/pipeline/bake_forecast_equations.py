...
vengeanceaiUSCMODEL6:
  • D&A on average PP&E so CapEx additions are depreciated
  • Gross AR days; Deferred Revenue separate in WC / NWC
  • SGA floor 20% with milder grind
"""
...
def _da_avg_ppe(col: str) -> str:
    """D&A = ((Open + Open+CapEx)/2) × DA%  — depreciates new CapEx."""
...
def bake_forecast_equations(wb) -> None:
    ...
    ws["B9"] = f"SGA % of Revenue (floor {floor:.0%}, −{SGA_IMPROVEMENT_BPS:.0f}bps×t)"
    ws["B10"] = "R&D % of Revenue (equation)"

    for col in _FORECAST_COLS:
        # Rate still from FY25 DA / FY25 PP&E; applied to AVERAGE PP&E in $ rows
        ...
        # GROSS AR days — I42 is gross AR in MODEL6
    ...
    for col in _FORECAST_COLS:
        ...
        # D&A on average PP&E (must match schedule row 94)
    ...
    _c_notes = {
        ...
        28: "eqn: Rev × SGA%",
        29: "eqn: Rev × R&D%",
        30: "eqn: AvgPPE × DA%  — Avg=(Open+Open+CapEx)/2",
    ...
    # ===== BALANCE SHEET =====
    # Gross AR; Total Liab = AP + Debt + Deferred (WC row 88)
    for col in _FORECAST_COLS:
        prev = _PREV[col]
        _formula(ws[f"{col}41"], f"={col}78", "#,##0.0")
        _formula(ws[f"{col}42"], f"={col}24*{col}15/365", "#,##0.0")  # gross AR
        ...
        # Include deferred revenue liability from WC schedule
    ...
    for col in _FORECAST_COLS:
        ...
        # ΔNWC is row 90 (= NWC_t − NWC_t−1); CF pulls that change
    ...
    for r, note in (
        (62, "eqn: EBT × (1 − tax%)"),
        (63, "eqn: AvgPPE × DA%"),
        (64, "eqn: NWC_t − NWC_t−1  (NWC=AR+Inv−AP−Deferred)"),
        (65, "eqn: NI + DA − ΔNWC"),
        (68, "eqn: Rev × CapEx%"),
    ...
    # ===== WC SCHEDULE: Gross AR, Inv, AP, Deferred, NWC, ΔNWC =====
    ...
    for col in _FORECAST_COLS:
        prev = _PREV[col]
        _formula(ws[f"{col}85"], f"={col}42", "#,##0.0")  # gross AR
        _formula(ws[f"{col}86"], f"={col}43", "#,##0.0")
        _formula(ws[f"{col}87"], f"={col}48", "#,##0.0")
        # Deferred = Rev × (FY25 Deferred / FY25 Rev)
        _formula(ws[f"{col}88"], f"={col}24*IF($I$24=0,0,$I$88/$I$24)", "#,##0.0")
        _formula(ws[f"{col}89"], f"={col}85+{col}86-{col}87-{col}88", "#,##0.0")
        _formula(ws[f"{col}90"], f"={col}89-{prev}89", "#,##0.0")

        # PPE schedule — D&A on average base
        if col == "J":
    ...
    ws["C88"] = "eqn: Rev × (FY25 Deferred/FY25 Rev) — separate from AR"
    ws["C88"].font = EQ_FONT
    ws["C89"] = "eqn: GrossAR + Inv − AP − Deferred"
    ws["C89"].font = EQ_FONT
