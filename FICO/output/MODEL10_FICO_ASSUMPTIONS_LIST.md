# vengeanceaiUSCMODEL10 — Assumptions List

**PDF download:** https://raw.githubusercontent.com/vengeanceaiUSC/Rainmaker/cursor/vengeanceaiusmodel10-d3ac/FICO/output/MODEL10_FICO_ASSUMPTIONS_LIST.pdf

Plain list only (no charts). Paste into Google Docs via File → Open if needed.

> Working Capital uses bottom-up DSO and DPO historical drivers. NWC = AR + Inventory − AP − Deferred Revenue (output); ΔNWC = NWCt − NWCt−1.
...
- **Why this choice:** MODEL10: restore software operating leverage. Floor 15%; grind −75 bps/yr so SG&A can scale toward mid-teens as revenue expands (not stuck ~24–25%).
...
- **Why this choice:** MODEL10: D&A is forecast as a % of Revenue (3yr FY23–25 average of total DepreciationDepletionAndAmortization / Revenue ≈ 0.84%), not a PP&E-only rate — capturing software amortization and intangibles.
...
- **Why this choice:** MODEL10: Working Capital uses bottom-up DSO and DPO historical drivers. NWC = AR + Inventory − AP − Deferred Revenue (output); ΔNWC = NWCt − NWCt−1. Row 15 is DSO again (not NWC%). AR is organic from collections days — no top-down NWC% AR plug.
...
- **Why this choice:** MODEL10: DPO is an explicit WC driver paired with DSO. NWC = AR + Inventory − AP − Deferred (output); ΔNWC = NWCt − NWCt−1.
...
- **Why this choice:** MODEL10: CapEx uses a flat 3yr average of (PPE purchases + capitalized software) / Revenue (≈ 1.25%). The prior hardcoded fade to 1% was removed so reinvestment reflects actual history rather than forcing CapEx ≈ D&A by Year 5.
...
- **Why this choice:** MODEL10: Stock-Based Compensation is added back in CFO and FCFF at the 3yr average SBC/Revenue (≈ 8.25%) from ShareBasedCompensation in the 10-K cash flow. SBC is already in operating expenses on the IS; adding it back in CF/FCFF avoids understating cash generation.
